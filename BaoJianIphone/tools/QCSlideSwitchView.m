...
- (void)createNameButtons
{
    ...
    //顶部tabbar的总长度
    CGFloat topScrollViewContentWidth = SCREENWIDTH;
    
    CGFloat length = 0;
    for (int i = 0; i < [_viewArray count]; i++) {
        UIViewController *vc = _viewArray[i];
        
         CGSize textSize = [vc.title getcontentsizeWithfont:UIFontHiraginoSansGBW3(15) constrainedtosize:CGSizeMake(150, 44) linemode:NSLineBreakByCharWrapping];
        length+=textSize.width;
    }
    
    widthofbutton = (SCREENWIDTH-length)/(([_viewArray count]+1)*1.0);
    
    //每个tab偏移量
    CGFloat xOffset = widthofbutton;
...
//        topScrollViewContentWidth += kWidthOfButtonMargin+textSize.width;
        ...
        //计算下一个tab的x偏移量
        xOffset += textSize.width + widthofbutton;
        
        [button setTag:i+100];
        if (i == 0) {
            _shadowImageView.frame = CGRectMake(widthofbutton-5, 10, textSize.width+10, _shadowImage.size.height);
            button.selected = YES;
        }
        [button setTitle:vc.title forState:UIControlStateNormal];
        button.titleLabel.font = UIFontHiraginoSansGBW3(15);
...
- (void)selectNameButton:(UIButton *)sender
{
    ...
    if (!sender.selected) {
        ...
        [UIView animateWithDuration:0.25 animations:^{
            ...
            [_shadowImageView setFrame:CGRectMake(sender.frame.origin.x-5, 10, sender.frame.size.width+10, _shadowImage.size.height)];
